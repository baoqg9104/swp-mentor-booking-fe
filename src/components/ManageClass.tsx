...
export default function ManageClass() {
  ...
  const [specClass, setSpecClass] = useState<number>();
  ...
  function openDelModal(n: number) {
    setDel(true);
    setSpecClass(n);
  }
  ...
  const deleteClass = async (id: number) => {
    try {
      const response = await axiosInstance
        .delete(`https://localhost:7007/api/SwpClass/delete/${id}`)
        .then((response) => {
      ...
      <div className="px-2 grid grid-cols-1 gap-4 py-2">
        <div>
          <button
            ...
            className="flex justify-center items-center px-2 py-2 border-4 rounded-lg align-items-center "
          >
